...
public class MegaCoffeeManager {
    ...
    private static void adminResult() {
        ...
        for (int i = 0; i < MENU_COUNT; i++) {
            if (product[i] != null) {
                String strMenu = (i + 1) + ". " + product[i] + " : " + sales[i];
                total = total + sales[i];
    ...
    private static void menuInsert() {
        ...
        System.out.println("현재 추가 할 수 있는 제품의 수 : " + (MENU_COUNT - ableCount));
        // 추가할 메뉴가 없으면 관리자 메뉴로 돌려보낸다
        ...
        if (ableCount >= 2) {
            ...
            switch (yesOrNo) {
                case "Y":
                    String newMenu = "";
                    int newPrice = 0;
                    System.out.println("새로 추가할 메뉴 명을 입력하세요");
                    newMenu = sc.next();
                    System.out.println("메뉴의 가격을 입력하세요");
                    newPrice = sc.nextInt();

                    // 새로운 메뉴를 배열에 추가한다.(product, price)
                    // 어디에 넣을까
                    // for loop로 추가하기
                    for (int i = 0; i < MENU_COUNT; i++) {
                        if (product[i] == null) {
    ...
    private static void menuUpdate() {
        while (true) {
            ...
            // 현재의 아이스아메리카노 가격은 2000원 입니다. -> 출력
            if (updateMenuNumber == 0) return;
            if (updateMenuNumber >= 1 && updateMenuNumber <= MENU_COUNT) {
                if (product[updateMenuNumber - 1] != null) {
                    // 존재하는 메뉴를 선택한 경우
                    System.out.println("현재의 " + product[updateMenuNumber - 1] + "가격은" + price[updateMenuNumber - 1] + "원 입니다.");
                    //변경한 단가를 요구한다
    ...
    private static void order() {
        while (true) {
            ...
            if (orderNumber >= 1 && orderNumber <= MENU_COUNT) {
                // 선택한 메뉴 처리
                // 메뉴에 있는 번호인지, 없는 번호인지 체크
                if (product[orderNumber - 1] != null) {
                    System.out.println("주문이 완료되었습니다. 감사합니다");
                    System.out.println(product[orderNumber - 1] + " : " + "받아가세요");
                    // 판매 금액을 처리한 후 메인 메뉴로 돌아가야함.
                    sales[orderNumber - 1] = sales[orderNumber - 1] + price[orderNumber - 1];
                    System.out.println("누적 판매 금액 : " + sales[orderNumber - 1]);
                } else {
    ...
    private static void viewMenu() {
        ...
        for (int i = 0; i < MENU_COUNT; i++) {
            if (product[i] != null) {
                String strMenu = (i + 1) + ". " + product[i] + " : " + price[i];
